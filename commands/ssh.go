...
func SSHConnect(db *gorm.DB, user models.User, logger slog.Logger, params string) error {
	...
	if forcedHost.ResolveTo != "" {
		sshHost = forcedHost.ResolveTo
	}
...
func resolveForcedHost(db *gorm.DB, user models.User, forcedHostname string) (models.Aliases, error) {
	...
	err := db.
		Where("LOWER(hostname) = ? AND user_id = ?", strings.ToLower(forcedHostname), user.ID).
		First(&host).Error
	if err == nil {
		return host, nil
	...
	err = db.
		Where("LOWER(hostname) = ? AND group_id IN (?)", strings.ToLower(forcedHostname), groupIDs).
		First(&host).Error
