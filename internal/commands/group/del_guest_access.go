...
// DelGuestAccess removes a guest access grant from a group.
func DelGuestAccess(db *gorm.DB, currentUser *models.User, args []string) error {
	fs := flag.NewFlagSet("groupDelGuestAccess", flag.ContinueOnError)
	var groupName, account, grantIDStr string
	fs.StringVar(&groupName, "group", "", "Group name")
	fs.StringVar(&account, "account", "", "Username whose guest access to remove")
	fs.StringVar(&grantIDStr, "grant", "", "Grant ID to remove (from groupListGuestAccesses)")
	var flagOutput bytes.Buffer
	fs.SetOutput(&flagOutput)

	if err := fs.Parse(args); err != nil || groupName == "" || account == "" {
		console.DisplayBlock(console.ContentBlock{
			Title:     "Delete Guest Access",
			BlockType: "error",
			Sections:  []console.SectionContent{{SubTitle: "Usage", Body: []string{"Usage: groupDelGuestAccess --group <group> --account <user> [--grant <grant_id>]"}}},
		})
		return err
	}

	if !currentUser.CanDo(db, "groupDelGuestAccess", groupName) {
		console.DisplayBlock(console.ContentBlock{
			Title:     "Delete Guest Access",
			BlockType: "error",
	...
	result := query.Delete(&models.GroupGuestAccess{})
	if result.Error != nil {
		console.DisplayBlock(console.ContentBlock{
			Title:     "Delete Guest Access",
			BlockType: "error",
			Sections:  []console.SectionContent{{SubTitle: "Database Error", Body: []string{"Error deleting guest access."}}},
		})
		return result.Error
	}
	if result.RowsAffected == 0 {
		console.DisplayBlock(console.ContentBlock{
			Title:     "Delete Guest Access",
			BlockType: "error",
			Sections:  []console.SectionContent{{SubTitle: "Not Found", Body: []string{"No matching guest access grant found."}}},
		})
		return nil
	}
	...
	console.DisplayBlock(console.ContentBlock{
		...
		Sections: []console.SectionContent{{SubTitle: "Success", Body: []string{
			fmt.Sprintf("Removed %d guest access grant(s) for '%s' in group '%s'.", result.RowsAffected, account, groupName),
		}}},
	})
	return nil
}
