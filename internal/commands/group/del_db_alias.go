...
// DelDBAlias removes a database alias from a group.
func DelDBAlias(db *gorm.DB, currentUser *models.User, args []string) error {
	fs := flag.NewFlagSet("groupDelDBAlias", flag.ContinueOnError)
	var groupName, aliasID string
	fs.StringVar(&groupName, "group", "", "Group name")
	fs.StringVar(&aliasID, "id", "", "Alias ID")
	var flagOutput bytes.Buffer
	fs.SetOutput(&flagOutput)

	if err := fs.Parse(args); err != nil || strings.TrimSpace(groupName) == "" || strings.TrimSpace(aliasID) == "" {
		console.DisplayBlock(console.ContentBlock{
			Title:     "Delete Group DB Alias",
			BlockType: "error",
			Sections:  []console.SectionContent{{SubTitle: "Usage", Body: []string{"Usage: groupDelDBAlias --group <group_name> --id <alias_id>"}}},
		})
		return err
	}

	if !currentUser.CanDo(db, "groupDelDBAlias", groupName) {
		console.DisplayBlock(console.ContentBlock{
			Title:     "Delete Group DB Alias",
			BlockType: "error",
	...
	if err := db.Where("id = ? AND group_id = ?", parsedID, group.ID).First(&alias).Error; err != nil {
		console.DisplayBlock(console.ContentBlock{
			Title:     "Delete Group DB Alias",
			BlockType: "error",
