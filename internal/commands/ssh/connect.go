...
type sshLogTarget struct {
	user string
	host string
	port string
}

func buildSSHLogger(base slog.Logger, sessionUser, sshFrom, protocol string, requested, effective sshLogTarget, jumpChain string) *slog.Logger {
	log := base.With(
		slog.String("user", sessionUser),
		slog.String("from", sshFrom),
		slog.String("requested_target_user", requested.user),
		slog.String("requested_target_host", requested.host),
		slog.String("requested_target_port", requested.port),
		slog.String("target_user", effective.user),
		slog.String("target_host", effective.host),
		slog.String("target_port", effective.port),
	)
	if protocol != "" {
		log = log.With(slog.String("protocol", protocol))
	}
	if jumpChain != "" {
		log = log.With(slog.String("jump_chain", jumpChain))
	}
	return log
}
...
func Connect(db *gorm.DB, user models.User, logger slog.Logger, params string) error {
	...
	requestedTarget := sshLogTarget{user: sshUser, host: sshHost, port: sshPort}
	...
	jumpChain := formatHopChain(hops)
	log := buildSSHLogger(logger, user.Username, sshFrom, protocol, requestedTarget, effectiveTarget, jumpChain)
...
func SFTPSession(db *gorm.DB, user models.User, logger slog.Logger, params string) error {
	...
	requestedTarget := sshLogTarget{user: sshUser, host: sshHost, port: sshPort}
	effectiveTarget := requestedTarget
	log := buildSSHLogger(logger, user.Username, sshFrom, "", requestedTarget, effectiveTarget, "")
