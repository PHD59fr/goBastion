...
func formatValue(v any, colName string, boolCols map[string]bool, targetDialect string) string {
	...
	case string:
		// PEM blocks (SSH private keys) must end with a trailing newline so that
		// OpenSSH can parse the temp file written during connection. SQLite stores
		// the key as-is from ssh-keygen (ending with \n), but the export/import
		// cycle can silently drop it. Restore it here for all PEM blocks.
		if strings.HasPrefix(val, "-----BEGIN ") && !strings.HasSuffix(val, "\n") {
			val = val + "\n"
		}
		// For PostgreSQL, use E-string syntax when the value contains newlines,
		// carriage returns, or backslashes. This avoids embedding raw newlines
		// inside string literals, which makes the SQL file immune to line-ending
		// corruption during file transfer (the root cause of "error in libcrypto"
		// when PEM private keys are migrated from SQLite to PostgreSQL).
		if targetDialect == "postgres" && strings.ContainsAny(val, "\r\n\\") {
			return "E'" + escapeSQLStringPostgres(val) + "'"
		}
...
// escapeSQLStringPostgres escapes a string for use inside a PostgreSQL E'...' literal.
// Backslashes must be escaped first, then single quotes, then control characters
// that must not appear as raw bytes in the SQL text (newline, carriage return).
func escapeSQLStringPostgres(s string) string {
