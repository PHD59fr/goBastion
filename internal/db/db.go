...
func ensureMySQLBusinessIdentityIndexes(db *gorm.DB) error {
	identities := []struct {
		...
		{"users", "active_username_hash", "uq_active_users_username", "CASE WHEN deleted_at IS NULL THEN UNHEX(SHA2(LOWER(username), 256)) ELSE NULL END"},
		{"user_groups", "active_membership_hash", "uq_active_user_groups_membership", "CASE WHEN deleted_at IS NULL THEN UNHEX(SHA2(CONCAT(user_id, ':', group_id), 256)) ELSE NULL END"},
		{"ingress_keys", "active_fingerprint_hash", "uq_active_ingress_keys_fingerprint", "CASE WHEN deleted_at IS NULL THEN UNHEX(SHA2(CONCAT(user_id, ':', fingerprint), 256)) ELSE NULL END"},
		{"aliases", "active_user_alias_hash", "uq_active_aliases_user", "CASE WHEN deleted_at IS NULL AND user_id IS NOT NULL THEN UNHEX(SHA2(CONCAT(user_id, ':', LOWER(resolve_from)), 256)) ELSE NULL END"},
		{"aliases", "active_group_alias_hash", "uq_active_aliases_group", "CASE WHEN deleted_at IS NULL AND group_id IS NOT NULL THEN UNHEX(SHA2(CONCAT(group_id, ':', LOWER(resolve_from)), 256)) ELSE NULL END"},
		{"database_aliases", "active_user_alias_hash", "uq_active_database_aliases_user", "CASE WHEN deleted_at IS NULL AND user_id IS NOT NULL THEN UNHEX(SHA2(CONCAT(user_id, ':', LOWER(resolve_from)), 256)) ELSE NULL END"},
		{"database_aliases", "active_group_alias_hash", "uq_active_database_aliases_group", "CASE WHEN deleted_at IS NULL AND group_id IS NOT NULL THEN UNHEX(SHA2(CONCAT(group_id, ':', LOWER(resolve_from)), 256)) ELSE NULL END"},
	}

	for _, identity := range identities {
		if !db.Migrator().HasColumn(identity.table, identity.column) {
			statement := fmt.Sprintf(
				"ALTER TABLE `%s` ADD COLUMN `%s` BINARY(32) GENERATED ALWAYS AS (%s) VIRTUAL",
