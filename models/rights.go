...
func (u *User) isGroupManager(db *gorm.DB) bool {
	var ug UserGroup
	if err := db.Where("user_id = ?", u.ID).First(&ug).Error; err != nil {
		return false
	}
	return ug.IsOwner() || ug.IsACLKeeper() || ug.IsGateKeeper()
}

func (u *User) isGroupMember(db *gorm.DB) bool {
	var ug UserGroup
	if err := db.Where("user_id = ?", u.ID).First(&ug).Error; err != nil {
		return false
	}
	return ug.IsMember() || ug.IsOwner() || ug.IsACLKeeper() || ug.IsGateKeeper()
}
